...
## P.15 問題1.5 Z(x,η) とするのは矛盾である  '25 6.12 {#C1_P15_問1.5_10}

$Z(x,y)=x^2e^y$ とする

$\eta = y-x$ とする

$Z(x,\eta)=x^2e^{\eta+x}$ とする。
...
そもそも異なる関数を同じ関数 $Z$ としているのは矛盾である

次に $(1)$ かつ $(3)$ も矛盾している。なぜなら

$(1)$ より $Z$ は関数なので任意の$x,y$ で $Z=x^2e^y$

$(3)$ より $Z=Z(1,1)=e$

よって 任意の $x,y$ で $e=Z=x^2e^y$
...
依存関係のある変数を関数の変数に代入するときは脳内で区別を行う必要がある
...
この問題のように関数1 $\rightarrow$ 変数 $\rightarrow$ 関数2 としてすべて同じ記号であらわすと脳内で区別がつかなくなって矛盾した結論をだすことがある

上の例だと

$\Zb(x,\eta) =  \Zm(x,\eta)$

$\so x^2e^\eta =x^2e^{\eta+x}$
