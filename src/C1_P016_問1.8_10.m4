...
## P.16 問題1.8 偏微分でつまづいたこと '25 6.25 {#C1_P016_問1.8_10}
...
$x, y$ が 従属変数であっても、$(x,y)$ 近傍で $f(x,y)$ が定義されるならば
偏微分の定義は成立するが、これを許容すると、$(x,y)$ 近傍で 従属変数$x,y$ が定義されないだけの場合も $f(x,y)$ が定義されずに偏微分できなくなって不便である
...
としているので $\red{8.}$ よりこれら$L$は異なる関数である。よって$\red{7.}$より矛盾する
